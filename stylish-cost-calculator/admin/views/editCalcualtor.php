                        ...
                        foreach ( $section->subsection as $sub ) {
							...
							<div class="boardOption">
								...
								<div class="subsection-area BodyOption ">
									<div class="scc-section-connection-line scc-link-line scc-hidden"></div>
									<?php
									$scc_total_elements_loop = count($sub->element);
									$scc_current_loop_index = 0;
                                    foreach ( $sub->element as $el ) {
										$scc_current_loop_index++;
										$scc_special_loop_class = 'scc-hidden';
										$scc_special_loop_element = '<div class="scc-line-hider"></div>';
										if ($scc_current_loop_index == 1) {
											$scc_special_loop_class = 'scc-first-element-connector scc-hidden';
											$scc_special_loop_element = '<div class="scc-line-hider scc-first-element-subsection"></div>';
										} elseif ($scc_current_loop_index == $scc_total_elements_loop) {
											$scc_special_loop_class = 'scc-last-element-connector scc-hidden';
											$scc_special_loop_element = '<div class="scc-line-hider scc-last-element-subsection"></div>';
										}
										?>
										<?php
                                        /**
                                        ...
                                        if ( $el->type == 'custom math' ) {
											...
											<div class="elements_added">
												...
												<div class="elements_added_v2">
													...
													<div class="element-title-desc" onclick="javascript:collapseElementTitle(this)" style="cursor: pointer;">
														<div class="title-desc-wrapper">
															<span class="element-title">Fee & Discount Adjuster</span>
															<p class="element-description">
																<?php echo esc_attr( $truncatedTitleElement ); ?>
															</p>
														</div>
												...
												<div class="scc-element-content" value="selectoption" style="display:none; height:auto">
													...
													<div class="edit-element-notice" style="margin-left: 15px;">
														<i class="material-icons with-tooltip" title="Note: Fee & Discount Adjuster applies extra calculation over the value returned by the elements in a subsection. E.g. If the subsection returns 100, and the custom math is 10%, the final value of the subsection will be 110. You can use to decrease the subsection total using negative sign to the value, for example, -10%">help_outline</i>
													</div>
													<!-- ELEMENT -->
...
<script type="text/javascript">
	...
	const debugMessages = (data, isCloudFlare = false) => {
		...
		let messages = {
			has_corrupted_database: {
				title: 'Database Tables Missing',
				message: msgs?.has_corrupted_database_message,
				show: msgs?.has_corrupted_database == true
			},
	...
	jQuery(document).ready(function() {
		<?php
		$installation_timestamp = get_option( 'scc_installation_timestamp' );
		$skipped_welcome_modal = get_option( 'scc_skipped_welcome_modal', false );
		$is_within_seven_days = ( time() - $installation_timestamp ) < 604800;
		$show_welcome_modal = $is_within_seven_days && ! $skipped_welcome_modal;
        if ( ( get_option( 'df-scc-save-count' ) !== 0 && get_option( 'df-scc-save-count' ) >= scc_feedback_invocation( 'form' ) ) && scc_feedback_invocation( 'form' ) != 0 ) {
			...
			if ( $show_welcome_modal ) {
				echo "sccBackendUtils.showWelcomeModal();";
			}
        }
?>
				...
				if (datajson.passed == true) {
					...
					if (datajson.first_slider_use) {
						Swal.fire({
							...
							html: `<p> Please check this documentation in order to understand how the Slider works. </br><a class="text-dark bg-white" target="_blank" href="${ sccHelpdeskLinks[ 'tutorial-slider-sections-subsections' ] }">Slider Documentation</a></p>
	...
	function insertCustomMath(idnewElement) {
		...
		element += '            <i class="material-icons with-tooltip" title="Note: Fee & Discount Adjuster applies extra calculation over the value returned by the elements in a subsection. E.g. If the subsection returns 100, and the custom math is 10%, the final value of the subsection will be 110. You can use to decrease the subsection total using negative sign to the value, for example, -10%">help_outline</i>'
		element += '        </div>'
		element += '        <!-- ELEMENT -->'
		...
		element += '            <div class="col-xs-6 col-md-7" style="padding:0px;"><input type="text" class="input_pad inputoption_title" onkeyup="clickedTitleElement(this)" style="height:40px;width:100%;" placeholder="Title" value="Fee & Discount Adjuster Name"></div>'
		element += '        </div>'
