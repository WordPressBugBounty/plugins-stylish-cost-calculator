...
$sccConfig                                                            = [
    ...
    'userActionBtns'                    => [
                ...
                'isWoocommerceBtnEnabled'  => $form->isWoocommerceCheckoutEnabled === 'true' ? true : false,
                'isDetailedListBtnEnabled' => $form->turnviewdetails !== 'true',
                'isEmailQuoteBtnEnabled'   => $form->turnoffemailquote !== 'true',
                'isStripeBtnEnabled'       => false,
                'isPaypalBtnEnabled'       => false,
            ],
...
foreach ( $form->sections as $index => $section ) {
    ...
    ?>
		...
		<?php if ( ! $hasAccordion ) { ?>
			...
			<?php } ?>
		<?php }
		$class_names = ['description_section_preview'];

    // Conditionally add class names based on $hasAccordion
    if ( $hasAccordion ) {
        $class_names[] = 'scc-accordion-panel';
        $class_names[] = 'section_' . esc_attr( $accordion_index );
    }
...
margin:0px;padding:0px;margin-top:0px;line-height:20px;vertical-align:middle;">
                            ...
                            foreach ( $el->elementitems as $item ) {
                                ...
                                ?>
										...
										<div class="simple_button control-label scc-form-field-item scc_add_btn_secn <?php echo esc_attr( $style_scc_calculator ) . '_button'; ?> " style="margin-top: unset;" >
											...
											<label class="checkbox-inline checkbox-inline-45 scc-simple-button-style">
        ...
        if ( $el->type == 'Dropdown Menu' ) {
            ?>
					<?php if ( $applyCustomColumn ) { ?>   
							<div id="ssc-elmt-<?php echo intval( $el->id ); ?>" data-typeElement="<?php echo esc_attr( $el->type ); ?>" class="scc-form-field-item clearfix has-custom-columns scc_dropdown_ 
														 <?php
						...
						<?php } else { ?>
							<div id="ssc-elmt-<?php echo intval( $el->id ); ?>" data-typeElement="<?php echo esc_attr( $el->type ); ?>" class="scc-form-field-item scc_dropdown_ 
														 <?php
					                                echo esc_attr( $style_scc_calculator );
					...
					<div id="ssc-elmt-<?php echo intval( $el->id ); ?>" data-typeElement="<?php echo esc_attr( $el->type ); ?>"  class="clearfix scc_sliderr__ scc-form-field-item 
					 ...
					 slider_in_mobile  scc_font_45">
						...
						<?php if ( $form->elementSkin == 'style_2' ) { ?>
							...
							<div class="scc-form-field-item scc-slider-with-input scc-slider-element-container">
            ...
            ?>
					...
					<div id="ssc-elmt-<?php echo intval( $el->id ); ?>" class="scc-form-field-item 
						...
						<div class="control-label <?php echo 'scc-col-sm-' . intval( $secondMobileColumnSize ) . ' scc-col-md-' . intval( $secondColumnSize ) . ' scc-col-lg-' . intval( $secondColumnSize ) . ' scc-p-0'; ?> scc_select_opt scc-form-field-item-control" style="padding:0px">
							<?php if ( $el->value2 <= 1 ) { ?>
								<input id="itemcreateds_0_0_0_<?php echo intval( $el->id ); ?>" class="comment_box_text itemCreated mandatory_no_45 scc-comment-box-input" data-inputtype="comment_input" onchange="triggerSubmit(7, this, <?php echo intval( $el->id ); ?>, 0, <?php echo intval( $form->id ); ?>)" placeholder="<?php echo esc_attr( $el->value3 ); ?>">
							<?php } else { ?>
...
>
				...
				<div class="control-label quantity-field-wrap scc-col-md-8 scc-col-lg-8 scc-col-xs-6 scc_select_opt scc-form-field-item-control" style="padding:0px;">
						<?php if ( $el->value1 == 'default' ) { ?>
							<input id="itemcreateds_0_0_0" <?php if ( $el->value4 && intval( $el->value4 ) > 0 ) { ?>
										onblur="triggerSubmit(3, this, <?php echo intval( $el->id ); ?>, 0, <?php echo intval( $form->id ); ?>)" 
									<?php } else { ?>
	...
	/*
		...
		*/
		...
		input#itemcreateds_0_0_0 {
			width: 120px;
			height: var(--scc-element-min-height);
		...
		.scc-simple-button-style{
			cursor:pointer;
			border: 1px solid <?php echo esc_attr( $colorObject ); ?>;
			font-size: 16px !important;
			color:<?php echo esc_attr( $colorObject ); ?>;
			user-select:none;
		}
		[id^="scc_form"] [id*=ssc-elmt-]:has(.scc-simple-button-style){
