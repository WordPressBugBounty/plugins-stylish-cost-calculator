...
if ( ! class_exists( 'googlefonts_scc' ) ) {
	class googlefonts_scc {
		...
		*/
		function __construct() {
			...
			//"Constants" setup
			$this->thisccuginurl  = WP_PLUGIN_URL . '/' . dirname( plugin_basename( __FILE__ ) ) . '/';
			$this->thisccuginpath = WP_PLUGIN_DIR . '/' . dirname( plugin_basename( __FILE__ ) ) . '/';
		...
		function scc_admin_scripts() {
			if ( isset( $_GET['page'] ) && $_GET['page'] == $this->gf_filename ) {
				$sc_scheme = get_option( 'df_scc_color-scheme', 1 );
				// wp_enqueue_style('scc-admin-style', SCC_URL  . 'styles/scc-front-end.css', array(), '3.1.1');
				// wp_enqueue_style('scc-admin-style', SCC_URL . 'styles/scc-back-end.css', array(), '3.1.1');
			}
		}
		...
		*/
		...
		function register_enqueue_style( $fonts = array() ) {
			$http = ( ! empty( $_SERVER['HTTPS'] ) ) ? 'https' : 'http';
			$url  = $http . '://fonts.googleapis.com/css';
			$url .= $this->get_api_query( $fonts );
			// wp_register_style('font-style-cc', $url);
			// wp_enqueue_style('font-style-cc');
			echo 'dikd we';
		}

		function style_late( $fonts = array() ) {
			$http = ( ! empty( $_SERVER['HTTPS'] ) ) ? 'https' : 'http';
			$url  = $http . '://fonts.googleapis.com/css';
			$url .= $this->get_api_query( $fonts );
		...
		*/
				...
				<table width="650" border="0" cellspacing="0" cellpadding="0">
				  <tr>
					<td>
						...
						<p><?php _e( 'This control panel gives you the ability to control how your Google Fonts fonts are disccayed. For more information about this plugin, please visit the', $this->localizationDomain ); ?>
		...
		function googlefont_action_callback() {
			...
			echo esc_html( $this->gf_get_font_selections( $name, $variants, $subsets ) );
			die();
		}
...
if ( class_exists( 'googlefonts_scc' ) ) {
	...
	$scc_googlefonts_var = new googlefonts_scc();
}
?>
