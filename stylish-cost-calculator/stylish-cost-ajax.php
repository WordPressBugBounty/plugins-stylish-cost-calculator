...
class ajaxRequest {
    ...
    public function skip_welcome_modal() {
        check_ajax_referer( 'edit-calculator-page', 'nonce' );
        update_option( 'scc_skipped_welcome_modal', true );
        wp_send_json( [ 'passed' => true ] );
    }
    ...
    private function generate_tutorial_email_from_wizard( $args ) {
        ...
        if ( $results_email_form_data['optin'] ) {
            ...
            return $this->generate_tutorial_pdf_from_wizard( $subject, $results_email_form_data['name'], $website_name, $template_collection );
        }
    }
    ...
    public function generate_tutorial_pdf_from_wizard( $title, $recipient_name, $website_name, $template_collection ) {
        ...
        $dompdf->setPaper( [0, 0, 595.28, 841.89 * 4], 'portrait' ); // A4 height * 20
    ...
    public function sccFeedbackManage() {
        if ( isset( $_REQUEST['source'] ) && ( $_REQUEST['source'] === 'plugins_page' ) ) {
    ...
    public function skip_premium_demo_modal() {
        check_ajax_referer( 'edit-calculator-page', 'nonce' );
        $current_save_count = intval( get_option( 'df-scc-save-count' ) );
        update_option( 'scc-skip-premium-demo-modal', $current_save_count + 20 );
        wp_send_json_success();
    }

    public function scc_ai_check_license() {
        check_ajax_referer( 'edit-calculator-page', 'nonce' );
        ...
        $api_endpoint = 'https://api.stylishcostcalculator.com/rest/ai-license';
    ...
    public function scc_ai_check_credits() {
        $page_name = sanitize_text_field( $_GET['page_name'] );
        check_ajax_referer( $page_name, 'nonce' );
        require_once __DIR__ . '/admin/controllers/ai-wizard-controller.php';
        $ai_builder   = new AiWizardController();

        $license_data = [
            'url'     => $ai_builder->get_base_url(),
        ];
        $api_endpoint = 'https://api.stylishcostcalculator.com/rest/ai-free-credits';
        $response     = wp_remote_post( $api_endpoint, [
            'method'  => 'POST',
            'headers' => ['Content-Type' => 'application/json'],
            'body'    => json_encode( $license_data ),
    ...
    public function sync_wizard_suggestions_state() {
        check_ajax_referer( 'edit-calculator-page', 'nonce' );
        ...
        if ( $method === 'read' ) {
            $current_wizard_data = $wpdb->get_var( "SELECT setup_wizard_data FROM {$wpdb->prefix}df_scc_forms WHERE id = $calc_id" );
            wp_send_json( $current_wizard_data );
        }

        if ( $method === 'save' ) {
            $wizard_suggestions_state = sanitize_text_or_array_field( file_get_contents( 'php://input' ) );
            // read current value of the setup_wizard_data column
            $current_wizard_data      = $wpdb->get_var( "SELECT setup_wizard_data FROM {$wpdb->prefix}df_scc_forms WHERE id = $calc_id" );
            $is_same_as_current_data  = $current_wizard_data === $wizard_suggestions_state;

            if ( ! $is_same_as_current_data ) {
                $query_res                = $wpdb->update( "{$wpdb->prefix}df_scc_forms", [ 'setup_wizard_data' => $wizard_suggestions_state ], [ 'id' => $calc_id ] );
            } else {
                wp_send_json_success( [
    ...
    public function scc_ai_wizard_request() {
        check_ajax_referer( 'edit-calculator-page', 'nonce' );
        $thread = '';
        ...
        $option_type   = sanitize_text_or_array_field( $request_data->type );
        $prompt        = sanitize_text_or_array_field( $request_data->prompt );
        $thread        = sanitize_text_or_array_field( $request_data->thread );
        $metadata      = sanitize_text_or_array_field( $request_data->metadata );
        ...
        $prompt_validation = true; //$ai_builder->ai_validate_prompt_length( $prompt );

        if ( $prompt_validation === false ) {
            $request_data = [
                'prompt'      => $prompt,
                'element_id'  => $element_id,
                'ai_response' => null,
                'type'        => $option_type,
                'error'       => 'Sorry, you can only use 400 characters in the prompt.',
            ];
        } else {
            if ( $option_type === 'suggest-elements' ) {
                $ai_response  = $ai_builder->suggest_elements( $option_type, $prompt, $thread, $metadata );
                $request_data = [
                    'prompt'        => $prompt,
            ...
            if ( $option_type === 'optimize-form' ) {
                $ai_response  = $ai_builder->optimize_form( $option_type, $prompt, $thread );
                $request_data = [
                    'prompt'        => $prompt,
                    'calculator_id' => $calculator_id,
                    'ai_response'   => $ai_response,
            ...
            if ( $option_type === 'setup-wizard' ) {
                $ai_response  = $ai_builder->setup_wizard( $option_type, $prompt, $thread );
                $request_data = [
                    'prompt'        => $prompt,
                    'calculator_id' => $calculator_id,
                    'ai_response'   => $ai_response,
